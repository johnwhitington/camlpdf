  ...
  in
    ...
    in
      ...
      in
        let new_trees =
          map
            (fun (name, trees) -> name, Pdftree.merge_name_trees_no_clash pdf trees)
            with_names
        in
  ...
  try
    ...
    let process_mark oldnums changes mark = 
      let rec pageobjectnumber_of_target t =
        ...
        | Pdfdest.StringDestination s -> pagenumber_of_target_string s
        | Pdfdest.Action a ->
            ...
            then
              let change_target_destinationpage target n =
                ...
                in
                  ...
                  | Pdfdest.StringDestination s -> Pdfdest.StringDestination s
...
let merge_pdfs retain_numbering do_remove_duplicate_fonts ?(process_struct_trees=true) ?(add_toplevel_document=false) names pdfs ranges =
          ...
          let namedict = merge_namedicts pdf pdfs in
            ...
            let extra_catalog_entries =
              if not process_struct_trees then extra_catalog_entries else
                match Pdfst.merge_structure_trees ~add_toplevel_document pdf pdfs with
                | None -> extra_catalog_entries
                | exception e ->
